...
export const Waveform = forwardRef((props, ref) => {
    ...
    const seekToPlayerAction = async (progress) => {
        if (mode === 'static') {
            try {
                ...
                if (seekSuccessful) {
                    ...
                    return Promise.reject(new Error(`error in seeking player to: ${progress}`));
                }
            }
            catch (error) {
            ...
            return Promise.reject(new Error('error in seekting to player: mode is not static'));
        }
    };
